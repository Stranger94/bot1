...
async def bump_adsites2():
    await bot.wait_until_ready()
    while not bot.is_closed:
        channel2 = bot.get_channel("553179703255760916")
        await bot.send_message(channel2, '!dl bump')                         
        await asyncio.sleep(7230)
...
async def invites(ctx, user: discord.Member):
        ...
        for invite in invite:  
            if invite.inviter == user:
                await bot.say("{} invited ".format(user.name) + str(invite.uses) + " people into this server!")
                x = x + 1
        if x == 0:
